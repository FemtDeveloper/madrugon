    ...
    >
      <div className=" flex flex-col justify-center items-center md:gap-8 bg-white rounded-2xl p-6 md:p-8 w-full h-full md:w-1/2 md:h-1/3">
        {modalContent && typeof modalContent !== "object" ? (
          // If modalContent is a ReactNode (e.g. component), render it directly
          (modalContent as any)
        ) : (
          <div className="flex flex-col gap-3 md:gap-4  items-center justify-center">
            <h3 className="h3 font-bold text-primary">
              {(modalContent as any)?.title}
            </h3>
            <h5 className="text-primary b1">
              {(modalContent as any)?.description}
            </h5>
            <div className="flex gap-4 mt-4">
              <CustomButton
                ...
                size="small"
              />
              {(modalContent as any) && (modalContent as any).onConfirm && (
                <CustomButton
                  btnTitle="Continuar"
                  onClick={() => {
                    (modalContent as any).onConfirm?.();
                    closeModal();
                  }}
                  size="small"
                />
