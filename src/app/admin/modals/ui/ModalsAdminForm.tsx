...
import { createPromoModal, type PromoModalInput } from "@/services/cms";
...
const schema = z.object({
  ...
  show_once_per_session: z.enum(["true", "false"]),
  user_id: z.string().uuid().optional().nullable(),
});

type FormData = z.infer<typeof schema>;

export const ModalsAdminForm = () => {
  ...
  const { mutate: createMut, isPending } = useMutation({
    mutationFn: (v: FormData) =>
      createPromoModal({
        ...
        show_once_per_session: v.show_once_per_session === "true",
        user_id: v.user_id ?? null,
      } as PromoModalInput),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["promo_modals"] });
      reset();
    },
  });

  const onSubmit = (v: FormData) => createMut(v);
    ...
    <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleSubmit(onSubmit)}>
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          {imageUrl && (
            <a className="text-primary underline b2" href={imageUrl} target="_blank" rel="noopener noreferrer">
              Ver
            </a>
          )}
        </div>
      ...
      <RHFCustomInput control={control} name="cta_url" label="URL del CTA" placeholder="/promos" />

      <RHFCustomInput control={control} name="position" label="Posición" placeholder="0" />
      ...
      <Controller
        ...
        render={({ field }) => (
          <div className="flex flex-col gap-1">
            <label className="b2 font-semibold">Mostrar una vez por sesión</label>
            <select className="input" value={field.value} onChange={(e) => field.onChange(e.target.value)}>
              <option value="true">Sí</option>
              <option value="false">No</option>
            </select>
      ...
      <Controller
        ...
        render={({ field }) => (
          <div className="flex flex-col gap-1">
            ...
            <select className="input" value={field.value ?? ""} onChange={(e) => field.onChange(e.target.value || undefined)}>
      ...
      <div className="col-span-1 md:col-span-2">
        <CustomButton btnTitle={isPending ? "Guardando..." : "Crear modal"} btnType="submit" />
      </div>
    </form>
  );
};

export default ModalsAdminForm;
